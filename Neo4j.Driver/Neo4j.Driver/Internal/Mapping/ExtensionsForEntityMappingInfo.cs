...
internal static class ExtensionsForEntityMappingInfo
{
    public static EntityMappingInfo GetEntityMappingInfo<T>(this T target) where T : ICustomAttributeProvider
    {
        ...
        var entityMappingInfo = new EntityMappingInfo { Path = path, EntityMappingSource = EntityMappingSource.Property };
        var result = GetEntityMappingInfoAffectedByAttributes(entityMappingInfo, target);
        return result;
    }
        ...
        ICustomAttributeProvider provider)
    {
        ...
        {
            info.Path = sourceAttribute.EntityMappingInfo.Path;
            info.EntityMappingSource = sourceAttribute.EntityMappingInfo.EntityMappingSource;
            info.Explicit = true;
        }

        // check for MappingOptionalAttribute and MappingDefaultValueAttribute
        info.Optional = provider.IsDefined(typeof(MappingOptionalAttribute), false);
        ...
        info.DefaultValue = defaultValueAttribute?.DefaultValue;
        return info;
    }
}
