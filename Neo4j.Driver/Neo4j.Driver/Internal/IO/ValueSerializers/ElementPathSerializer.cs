...
{
    internal class ElementPathSerializer : ReadOnlySerializer
    {
        ...
        public override object Deserialize(IPackStreamReader reader, byte signature, long size)
        {
            ...
            for (var i = 0; i < segments.Length; i++)
            {
                ...
                if (relIdx < 0)
                {
                    ...
                    rel.SetStartAndEnd(nextNode.ElementId, prevNode.ElementId);
                }
                else
                {
                    rel = uniqRels[relIdx - 1];
                    rel.SetStartAndEnd(prevNode.ElementId, nextNode.ElementId);
                }
