...
namespace Neo4j.Driver.Tests.TestBackend
{
    internal class CypherTypeField : ProtocolObject
    {
        ...
        public override string Respond() =>
            new ProtocolResponse("Field", new {value = NativeToCypher.Convert(Field)})
                .Encode();
        ...
        private object ReadProperty(Type type, object obj, string field)
        {
            try
            {
                var property = type
                    .GetProperties()
                    .Single(x => string.Equals(x.Name.ToLower(), field.ToLower(), StringComparison.CurrentCultureIgnoreCase)); 
