...
namespace Neo4j.Driver
{
    ...
    public interface IRxSession : IRxRunnable
    {
        ...
        /// <summary>
        /// Begin a new <strong>explicit</strong> <see cref="IRxTransaction" />. Actual transaction is only created once
        /// an observer is subscribed to the returned reactive stream. A session instance can only have at most one transaction at
        /// a time. If you want to run multiple concurrent transactions, you should use multiple concurrent sessions.
        /// </summary>
        ...
        /// <see cref="TransactionConfig" /> that applies to the underlying auto-commit transaction. The query is only executed
        /// when an <see cref="IObserver{T}" /> is subscribed to one of the reactive streams that can be accessed through the
        /// returned reactive result.
        /// </summary>
        /// <param name="query">Query to be executed</param>
        /// <param name="action">
        /// Given a <see cref="TransactionConfigBuilder" />, defines how to create the configuration for the
