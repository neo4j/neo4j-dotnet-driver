...
{
	public class RxWriteCommandTxFunc<TContext> : RxCommand<TContext>
		where TContext : StressTestContext
	{
		private readonly StressTest<TContext> _test;

		public RxWriteCommandTxFunc(StressTest<TContext> test, IDriver driver, bool useBookmark)
		...
		public override async Task ExecuteAsync(TContext context)
		{
			...
			await session.WriteTransaction(txc => txc
					.Run("CREATE ()")
					.Consume()
