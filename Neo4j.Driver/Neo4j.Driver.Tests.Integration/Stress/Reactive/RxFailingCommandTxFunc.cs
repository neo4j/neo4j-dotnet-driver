...
{
	public class RxFailingCommandTxFunc<TContext> : RxCommand<TContext>
		where TContext : StressTestContext
	{
		public RxFailingCommandTxFunc(IDriver driver)
			: base(driver, false)
		{
		}
		...
		public override Task ExecuteAsync(TContext context)
		{
			...
			session.ReadTransaction(tx => tx
					.Run("UNWIND [10, 5, 0] AS x RETURN 10 / x")
					.Records()
					.Select(r => r[0].As<int>())
					.CatchAndThrow(exc => tx.Rollback<int>())
					.Concat(tx.Commit<int>()))
