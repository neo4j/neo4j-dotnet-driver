...
#region Receive Only Tests
    ...
    public async Task ShouldReceiveDuration()
    {
        await TestReceiveData(
            "RETURN duration({ months: 16, days: 45, seconds: 120, nanoseconds: 187309812 })",
    ...
    {
        await TestReceiveData(
            "RETURN date({ year: 1994, month: 11, day: 15 })",
    ...
    {
        await TestReceiveData(
            "RETURN localtime({ hour: 23, minute: 49, second: 59, nanosecond: 999999999 })",
...
#region Send and Receive System Types
    ...
    public async Task ShouldSendAndReceiveSystemDateTimeOffsetNanoseconds()
    {
        var data = new DateTimeOffset(
            new DateTime(1979, 2, 15, 7, 5, 25).AddTicks(748999900),
            TimeSpan.FromMinutes(-150));
...
#region Helper Methods
    ...
    private async Task TestSendAndReceiveArrayWithType<TSent, TRecv>(
        IEnumerable<TSent> value,
...
#region Random Temporal Value Generation
    ...
    private TimeSpan RandomSystemTime()
    {
        return new LocalTime(
            ...
            _random.Next(TemporalHelpers.MinMinute, TemporalHelpers.MaxMinute),
            _random.Next(TemporalHelpers.MinSecond, TemporalHelpers.MaxSecond),
            _random.Next(TemporalHelpers.MinNanosecond, TemporalHelpers.MaxNanosecond) / 100 * 100).ToTimeSpan();
    }
