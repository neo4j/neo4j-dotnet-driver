...
namespace Neo4j.Driver.Tests
{
    public class PackStreamMessageFormatV1Tests
    {
        public class WriterV1Tests
        {
            ...
            private class Mocks
            {
                //public Mock<MemoryStream> MockReadStream { get; }
                //public Mock<MemoryStream> MockWriteStream { get; }
                ...
                public Mocks()
                {
                    ...
                    MockTcpSocketClient
                        .Setup(t => t.ReadStream)
                        .Returns(ReadStream);
                }

                public void VerifyWrite(byte[] bytes, int bufferSize = Constants.BufferSize)
                {
                    //byte[] expectedBytes = bytes.PadRight(bufferSize);

                    Assert.Equal(bytes, WriteStream.ToArray());
            ...
            public void PacksInitMessageCorrectly()
            {
                ...
                var writer = new BoltWriter(mocks.WriteStream);
        ...
        public class ReaderV1Tests
        {
            public class UnpackValueMethod
            {
                public void ShouldPackBytes()
                {
                    var inputStreamMock = new Mock<Stream>();
                    inputStreamMock.Setup(x => x.CanRead).Returns(true);
                    inputStreamMock.SetupSequence(x => x.ReadByte()).Returns(PackStream.BYTES_8).Returns((byte)0x00);

                    var reader = new PackStreamReader(inputStreamMock.Object);

                    var unpackValue = reader.Read().ValueAs<byte[]>();
                ...
                private void UnpackNumCorrectly(long expected, byte[] data)
                {
                    ...
                    SetupClientWithReadStream(mockTcpSocketClient, data);

                    IPackStreamReader reader = new PackStreamReader(mockTcpSocketClient.Object.ReadStream);
