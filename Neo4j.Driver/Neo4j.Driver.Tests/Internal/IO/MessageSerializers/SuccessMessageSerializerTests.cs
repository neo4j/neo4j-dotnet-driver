...
namespace Neo4j.Driver.Internal.IO.MessageSerializers
{
    public class SuccessMessageSerializerTests
    {
        ...
        [InlineData(5, 0)]
        [InlineData(6, 0)]
        public void DeserializeShuldReadMap(int major, int minor)
        {
            ...
            psw.WriteDictionary(new Dictionary<string, object>() as IDictionary<string, object>);

            memory.Position = 0;
            var message = SuccessMessageSerializer.Instance.Deserialize(reader, 0x00, 0);
            
            message.Should().BeOfType<SuccessMessage>();
        }
    }
}
