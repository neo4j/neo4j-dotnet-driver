    ...
    public class RunResponseHandlerTests
    {
        ...
        public void ShouldCallRunCompletedOnSuccess()
        {
            ...
            handler.OnSuccess(new[]
                {FieldsCollectorTests.TestMetadata, ResultAvailableAfterCollectorTests.TestMetadata}.ToDictionary());

            streamBuilder.Verify(x => x.RunCompleted(-1, FieldsCollectorTests.TestMetadataCollected, null),
                Times.Once);

            summaryBuilder.VerifySet(
                x => x.ResultAvailableAfter = ResultAvailableAfterCollectorTests.TestMetadataCollected, Times.Once);
        }
